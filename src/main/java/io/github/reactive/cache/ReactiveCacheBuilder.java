...
import static io.github.reactive.cache.common.Assert.checkState;
...
public final class ReactiveCacheBuilder<K, V> {
    ...
    ReactiveCacheBuilder<K, V> keyEquivalence(Equivalence<Object> equivalence) {
        Assert.checkState(keyEquivalence == null, "key equivalence was already set to %s", keyEquivalence);
    ...
    public ReactiveCacheBuilder<K, V> concurrencyLevel(int concurrencyLevel) {
        ...
        Assert.checkArgument(concurrencyLevel > 0);
    ...
    public ReactiveCacheBuilder<K, V> maximumSize(long size) {
        ...
        Assert.checkState(
                ...
                this.maximumWeight);
        Assert.checkState(this.weigher == null, "maximum size can not be combined with weigher");
    ...
    public ReactiveCacheBuilder<K, V> maximumWeight(long weight) {
        ...
        Assert.checkArgument(weight >= 0, "maximum weight must not be negative");
        return this;
    }
    ...
    public <K1 extends K, V1 extends V> ReactiveCacheBuilder<K1, V1> weigher(
        ...
        Assert.checkState(this.weigher == null);
        if (strictParsing) {
            Assert.checkState(
    ...
    public ReactiveCacheBuilder<K, V> expireAfterWrite(long duration, TimeUnit unit) {
        ...
        Assert.checkArgument(duration >= 0, "duration cannot be negative: %s %s", duration, unit);
    ...
    public ReactiveCacheBuilder<K, V> refreshAfterWrite(long duration, TimeUnit unit) {
        ...
        Assert.checkState(refreshNanos == UNSET_INT, "refresh was already set to %s ns", refreshNanos);
        Assert.checkArgument(duration > 0, "duration must be positive: %s %s", duration, unit);
    ...
    public ReactiveCacheBuilder<K, V> loadingTimeout(long duration, TimeUnit unit) {
        Objects.requireNonNull(unit);
        Assert.checkState(timeoutNanos == UNSET_INT, "refresh was already set to %s ns", timeoutNanos);
        Assert.checkArgument(duration > 0, "duration must be positive: %s %s", duration, unit);
    ...
    public ReactiveCacheBuilder<K, V> timeoutScheduler(Scheduler scheduler) {
        ...
        Assert.checkState(Objects.isNull(timeoutScheduler), "timeout scheduler was already set");
    ...
    public ReactiveCacheBuilder<K, V> loadingRestartScheduler(Scheduler scheduler) {
        Objects.requireNonNull(scheduler);
        Assert.checkState(Objects.isNull(loadingRestartScheduler), "loading restart scheduler was already set");
    ...
    public ReactiveCacheBuilder<K, V> ticker(Ticker ticker) {
        Assert.checkState(this.ticker == null);
    ...
    public <K1 extends K, V1 extends V> ReactiveCacheBuilder<K1, V1> removalListener(
        ...
        Assert.checkState(this.removalListener == null);
    ...
    private void checkNonLoadingCache() {
        Assert.checkState(refreshNanos == UNSET_INT, "refreshAfterWrite requires a LoadingCache");
    }

    private void checkWeightWithWeigher() {
        if (weigher == null) {
            Assert.checkState(maximumWeight == UNSET_INT, "maximumWeight requires weigher");
        } else {
            if (strictParsing) {
                Assert.checkState(maximumWeight != UNSET_INT, "weigher requires maximumWeight");
            } else {
